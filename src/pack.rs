...
type AttrValue<'a> = gimli::AttributeValue<gimli::EndianSlice<'a, gimli::LittleEndian>, usize>;
pub type DebugFrame<'a> = gimli::DebugFrame<gimli::EndianSlice<'a, gimli::LittleEndian>>;
...
impl Pack {
    ...
    fn load_program_headers(&mut self, eid: usize, elf: &goblin::elf::Elf) -> Result<()> {
    ...
    fn load_sections(&mut self, eid: usize, elf: &goblin::elf::Elf, buff: &[u8]) -> Result<()> {
        ...
        for sh in &elf.section_headers {
            if let Some(seg_name) = elf.shdr_strtab.get_at(sh.sh_name) {
                ...
                if seg_name == id {
                    ...
                    self.frames
                        .insert(eid, buff[offset..offset + size].to_vec());
                }
            }
        }
        Ok(())
    ...
    fn make_gid(&mut self, eid: usize, unit: &Unit, entry: &DebugInfoEntry) -> Gid {
        use gimli::{DebugInfoOffset as DIO, DebugTypesOffset as DTO, UnitSectionOffset as USO};
        match entry.offset().to_unit_section_offset(unit) {
            USO::DebugInfoOffset(DIO(goff)) | USO::DebugTypesOffset(DTO(goff)) => Gid { eid, goff },
        }
    }
    ...
    fn insert_primitive(
        ...
        while let Some(attr) = attrs.next()? {
            match (attr.name(), attr.value()) {
                ...
                (gimli::DW_AT_name, v) => {
                    if let Ok(inner) = dwarf.attr_string(unit, v).and_then(|s| s.to_string()) {
                        name = Some(inner);
                    }
                }
    ...
    fn dwarf_value_to_gid(&mut self, eid: usize, unit: &Unit, attrval: &AttrValue) -> Option<Gid> {
        use gimli::{DebugInfoOffset as DIO, DebugTypesOffset as DTO, UnitSectionOffset as USO};
        match attrval {
            gimli::AttributeValue::UnitRef(unitoff) => match unitoff.to_unit_section_offset(unit) {
                USO::DebugInfoOffset(DIO(goff)) | USO::DebugTypesOffset(DTO(goff)) => {
                    Some(Gid { eid, goff })
                }
            },
            &gimli::AttributeValue::DebugInfoRef(DIO(goff)) => Some(Gid { eid, goff }),
            _ => None,
        }
    }
    ...
    fn insert_variable(
        ...
        while let Some(attr) = attrs.next()? {
            match attr.name() {
                ...
                gimli::DW_AT_type => typ = self.dwarf_value_to_gid(eid, unit, &attr.value()),
                _ => {}
            }
        }
    ...
    fn load_dwarf(&mut self, eid: usize, elf: &goblin::elf::Elf, buff: &[u8]) -> Result<()> {
        let dwarf_cow = gimli::Dwarf::<&[u8]>::load(|sec_id| {
            let sec = elf
                .section_headers
                .iter()
                .find(|sh| Some(sec_id.name()) == elf.shdr_strtab.get_at(sh.sh_name));
            if let Some(&goblin::elf::SectionHeader {
                sh_offset, sh_size, ..
            }) = sec
            {
                ...
                buff.get(start..end)
                    .ok_or_else(|| Error::BadSectionName(sec_id.name().to_string()))
            } else {
                Ok(&[])
            }
        })?;
        let dwarf =
            dwarf_cow.borrow(|section| gimli::EndianSlice::new(&section, gimli::LittleEndian));
        ...
        while let Some(header) = iter.next()? {
            ...
            while let Some((delta, entry)) = entries.next_dfs()? {
                ...
                match entry.tag() {
                    ...
                    gimli::DW_TAG_variable => self.insert_variable(eid, &dwarf, &unit, entry)?,
                    gimli::DW_TAG_pointer_type => self.insert_ptrtype(eid, &dwarf, &unit, entry)?,
                    ...
                    gimli::DW_TAG_union_type => self.insert_struct(eid, &dwarf, &unit, entry)?,
                    gimli::DW_TAG_member => {
                        let owner = gid_stack[depth - 1];
    ...
    fn load_symbols(&mut self, eid: usize, elf: &goblin::elf::Elf) -> Result<()> {
        for sym in elf.syms.iter() {
            match sym.st_type() {
                goblin::elf::sym::STT_NOTYPE
                | goblin::elf::sym::STT_SECTION
                | goblin::elf::sym::STT_FILE => continue,
                _ => {}
            }
            if let Some(name) = elf.strtab.get_at(sym.st_name) {
                ...
                self.elf_syms_byname
                    .entry(name.clone())
                    .or_default()
                    .insert(addr);
                self.elf_syms.entry(addr).or_default().push(Symbol {
                    eid,
                    name,
                    addr,
                    size,
    ...
    fn load_object(&mut self, eid: usize, elf: &goblin::elf::Elf, buff: &[u8]) -> Result<()> {
    ...
    pub fn dwarf_function(&self, name: &'_ str) -> Option<Vec<&Function>> {
        ...
        Some(
            addrs
                .iter()
                .filter_map(|a| self.dwarf_functions.get(a))
                .collect(),
        )
    }
    ...
    pub fn elf_symbol(&self, name: &'_ str) -> Option<Vec<&Symbol>> {
        ...
        Some(
            addrs
                .iter()
                .filter_map(|a| self.elf_syms.get(a))
                .flatten()
                .collect(),
        )
    }
        ...
        for (offset, mems) in members {
            for imem in mems {
                if imem.name.is_none() {
                    if let Some(s_mems) = self
                        .lookup_struct(imem.typ)
                        .and_then(|s| self.struct_members(s))
                    {
                        for (inner_offset, mut inner_mems) in s_mems {
                            total_members
                                .entry(inner_offset + offset)
                                .or_default()
    ...
    pub fn size_of(&self, gid: Gid) -> Option<usize> {
        match self.lookup_type(gid)? {
            ...
            Typ::Arr(arr) => {
                if let (Some(len), Some(size)) = (self.array_len(arr), self.size_of(arr.typ)) {
                    Some(size * len)
                } else {
                    None
                }
